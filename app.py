...
def normalize_product_name(name: Any) -> str:
    """Keep all legacy legacy outdoor-label records aligned to Outdoor."""
...
def add_missing_historical_product(inventory: pd.DataFrame, product_name: str, category: str, unit_price: float) -> Tuple[pd.DataFrame, str]:
    ...
    category = category if category in CATEGORIES else "Accessories / Other"
...
def build_historical_sales_from_rows(rows: pd.DataFrame, inventory: pd.DataFrame, existing_sales: List[Dict[str, Any]]) -> Tuple[List[Dict[str, Any]], pd.DataFrame, Dict[str, Any]]:
    ...
    for _, row in rows.iterrows():
        ...
        category = str(row.get("category", "Accessories / Other"))
        payment_method = str(row.get("payment_method") or "Cash")
        if payment_method == "Unknown":
            payment_method = "Cash"

        fingerprint = sale_import_fingerprint(timestamp, product_name, quantity, workbook_price, workbook_total)
...
def page_import_historical_sales(inventory: pd.DataFrame, sales: List[Dict[str, Any]]) -> None:
    hero("Import Historical Sales", "Load handwritten historical sales into the same POS sales history and stock flow.")

    st.info(
        "This import appends to saint_herb_sales.json, deducts stock, and prevents duplicate imports using row fingerprints. "
        "Imported sales then flow into Dashboard, Sales Reports, Top Selling Products, Product Analytics, and stock calculations."
    )
    ...
    uploaded = st.file_uploader("Upload historical sales Excel workbook", type=["xlsx", "xls"])
    if uploaded is None:
        st.caption("Expected workbook sheet: POS_Import_Format. Fallback supported: Transactions.")
        return

    try:
        rows = historical_import_rows_from_excel(uploaded)
    except Exception as exc:
        st.error(f"Could not read the workbook: {exc}")
        return

    st.subheader("Import Preview")
    preview_cols = [c for c in ["timestamp", "payment_method", "product_name", "quantity", "unit_price", "line_total", "source", "confidence"] if c in rows.columns]
    st.dataframe(rows[preview_cols].head(100), use_container_width=True, hide_index=True)

    summary_cols = st.columns(4)
    with summary_cols[0]:
        metric_card("Rows Found", f"{len(rows):,}", "Valid historical sale rows")
    with summary_cols[1]:
        metric_card("Units", f"{rows['quantity'].sum():,.0f}", "Workbook quantity")
    with summary_cols[2]:
        metric_card("Workbook Revenue", money(rows["line_total"].sum()), "Before POS price override")
    with summary_cols[3]:
        metric_card("Products", f"{rows['product_name'].nunique():,}", "Unique product names")

    st.warning("Import will append sales and deduct stock. It will not overwrite existing sales.")
    if st.button("Import Historical Sales", type="primary", use_container_width=True):
        new_sales, updated_inventory, import_summary = build_historical_sales_from_rows(rows, inventory, sales)
        if not new_sales:
            st.warning(f"No new sales imported. Duplicates skipped: {import_summary['skipped_duplicates']:,}.")
            return
        ...
        append_audit(
            "HISTORICAL_SALES_IMPORTED",
            "BATCH",
            "System",
            "Historical handwritten workbook imported",
...
def product_analytics_dataframe(inventory: pd.DataFrame, sales: List[Dict[str, Any]]) -> pd.DataFrame:
    ...
    product_perf = items.groupby(["product_id", "name"], as_index=False).agg(
        units_sold=("quantity", "sum"),
        revenue=("line_total", "sum"),
        last_sale_date=("date_parsed", "max"),
    )
    product_perf["average_selling_price"] = product_perf["revenue"] / product_perf["units_sold"].replace(0, pd.NA)
    total_revenue = product_perf["revenue"].sum()
    product_perf["revenue_contribution_pct"] = (product_perf["revenue"] / total_revenue * 100).fillna(0)

    inv_cols = ["id", "quantity_on_hand", "stock_value", "days_stock_on_hand", "status"]
    merged = product_perf.merge(inventory[inv_cols], left_on="product_id", right_on="id", how="left")
    today = date.today()
    merged["days_since_last_sale"] = merged["last_sale_date"].map(lambda d: (today - d).days if pd.notna(d) else None)
    merged["stock_turnover"] = merged["units_sold"] / merged["quantity_on_hand"].replace(0, pd.NA)
    return merged.drop(columns=["id"], errors="ignore")


def inventory_velocity_label(days_stock: float, units_sold: float, days_since_last_sale: Any) -> str:
    if safe_float(units_sold) <= 0:
        return "Dead Stock"
    if pd.notna(days_since_last_sale) and safe_float(days_since_last_sale) > 14:
        return "Slow Moving"
    if safe_float(days_stock) < 10:
        return "Low Stock"
    if safe_float(days_stock) > 45:
        return "Overstocked"
    return "Fast Moving"
